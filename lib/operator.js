...
proto.select = function* (table, options) {
  options = options || {};
  const sql = options.prefix || this._selectColumns(table, options.columns) +
    ...
    if (typeof value === 'object') {
      if (Array.isArray(value)) {
        ...
        if (typeof value.operator !== 'string') {
          throw new Error('Can not lack operator field in custom where.');
        }
        operator = [value.operator];
        value = [value.value];
      }
      else if (Array.isArray(value.operators)
        && Array.isArray(value.values)
        && value.operators.length === value.values.length
      ) {
...
proto._groups = function(groups) {
  ...
  for (let i = 0; i < groups.length; i++) {
    values.push(this.escapeId(groups[i]));
  }
